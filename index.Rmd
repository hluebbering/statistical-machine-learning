...
```{r}
source("data/data.R")
twtr.stock <- data.frame(twtr.stock)
tsla.stock <- data.frame(tsla.stock)

elonMusk.stocks <- tsla.stock # <- bind_rows(twtr.stock, tsla.stock)
elonMusk.stocks$direction <- as.factor(elonMusk.stocks$direction)
stocks.data <- elonMusk.stocks %>% dplyr::select(-adjusted)
```


```{r}
twitter.data <- read.csv("data/dfclean.csv")
twitter.data <- twitter.data %>% dplyr::select(created_at, status_id, text, favorite_count, retweet_count, quote_count, reply_count)
...
twitter.data2 <- twitter.data %>% dplyr::group_by(created_at) %>% 
  summarise(ntweets = n(),
...
df <- dplyr::inner_join(twitter.data2, elonMusk.stocks, by = c("created_at" = "date"))
...
> Stock Market Data for `Tesla` and `Twitter` between $2018$ and $2022$
...
pairs.plot3 <- ggplot(data = df.pairs, mapping = aes(x = nfav, y = daily_return)) + geom_point(shape = 21, fill = "#AFCB80", color = "#97BC62", alpha = 0.45, size = 2.5, stroke=0.4) + geom_smooth(cex = 1, alpha=0.15, color = "#759465", se = FALSE, method = "loess", span = 0.25) + xlab("lag3") + ylab("daily_return") + theme_ipsum_rc(plot_margin = ggplot2::margin(10, 10, 10, 10), base_size = 8) + theme(axis.title.y = element_text(size = 8), axis.text.y = element_text(size = 8), axis.text.x = element_text(size = 8), axis.title.x = element_text(size = 8))

pairs.plot4 <- ggplot(data = df.pairs, mapping = aes(x = nretweet, y = daily_return)) + geom_point(shape = 21, fill = "#AFCB80", color = "#97BC62", alpha = 0.45, size = 2.5, stroke=0.4) + geom_smooth(cex = 1, alpha=0.15, color = "#759465", se = FALSE, method = "loess", span = 0.25) + xlab("lag3") + ylab("daily_return") + theme_ipsum_rc(plot_margin = ggplot2::margin(10, 10, 10, 10), base_size = 8) + theme(axis.title.y = element_text(size = 8), axis.text.y = element_text(size = 8), axis.text.x = element_text(size = 8), axis.title.x = element_text(size = 8))

pairs.plot5 <- ggplot(data = df.pairs, mapping = aes(x = nreply, y = daily_return)) + geom_point(shape = 21, fill = "#AFCB80", color = "#97BC62", alpha = 0.45, size = 2.5, stroke=0.4) + geom_smooth(cex = 1, alpha=0.15, color = "#759465", se = FALSE, method = "loess", span = 0.25) + xlab("lag3") + ylab("daily_return") + theme_ipsum_rc(base_size = 8, plot_margin = ggplot2::margin(10, 10, 10, 10)) + theme(axis.title.y = element_text(size = 8), axis.text.y = element_text(size = 8), axis.text.x = element_text(size = 8), axis.title.x = element_text(size = 8))
...
```{r, out.width="32%", fig.show='hold', fig.align='default'}
ggplot(data = df.pairs, mapping = aes(x = adjusted)) + geom_histogram(binwidth = 18, color = "#97BC62", fill = "#C6E67A", cex=0.35) + 
...
```{r, echo=TRUE}
range(df.pairs$adjusted)
summary(df.pairs$adjusted)
...
```{r}
df.pairs2 <- df.pairs %>% dplyr::select(ntweets, nfav, nretweet, nreply, volume, adjusted, daily_return, lag1, lag2, years)
